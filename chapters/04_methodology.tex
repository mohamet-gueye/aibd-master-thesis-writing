...
\begin{table}[htbp]
    \centering
    \caption[Record counts through the data-validation pipeline]{Record counts through the data validation and passenger-filtering pipeline (\texttt{notebooks/preprocessing/02\_data\_validation.ipynb}).}
...
\begin{table}[htbp]
    \centering
    \caption[Assumed seat capacity by aircraft type]{Assumed seat capacity by aircraft type used for passenger estimation (\texttt{notebooks/preprocessing/02\_data\_validation.ipynb}).}
...
\begin{table}[htbp]
    \centering
    \caption[Passenger- and baggage-pressure classification rules]{Passenger- and baggage-pressure classification rules applied to hourly aggregated demand}
...
\begin{table}[htbp]
    \centering
    \caption[Passenger-pressure regression performance]{Regression performance for future passenger-pressure forecasting on the held-out test set ($n = 3{,}982$).}
...
\begin{table}[htbp]
  \centering
  \renewcommand{\arraystretch}{1.25}
  \caption[Full-feature congestion-classifier performance]{Full-feature congestion classifiers on the held-out test set ($n = 3{,}982$); \texttt{future\_baggage\_pressure} retained as a predictor.}
...
\begin{figure}[htbp]
  ...
  \includegraphics[
    width=\textwidth,
    keepaspectratio
  ]{figures/chapter4/fig_4_1_full_feature_importance_leakage.png}
  \caption[Full-feature Random Forest classification feature importance]{Top 15 classification features for the full-feature Random Forest
...
\begin{table}[htbp]
  \centering
  \renewcommand{\arraystretch}{1.25}
  \caption[Reduced-feature congestion-classifier performance]{Reduced-feature congestion classifiers on the held-out test set ($n = 3{,}982$); \texttt{future\_baggage\_pressure} excluded.}
...
\begin{figure}[htbp]
  ...
  \includegraphics[
    width=\textwidth,
    keepaspectratio
  ]{figures/chapter4/fig_4_2_classifier_performance_comparison.png}
  \caption[Full-feature and reduced-feature classifier performance]{Accuracy, recall, F1 score, and ROC-AUC for the full-feature Random
...
\begin{table}[H]
    \centering
    \caption[AnyLogic operational-period lookup table]{Operational-period lookup table used to drive time-varying processing rate and congestion-risk weighting in the AnyLogic model (\texttt{data/processed/anylogic/anylogic\_operational\_periods.csv}).}
...
\begin{table}[H]
    \centering
    \caption[AnyLogic scenario-parameter configuration]{AnyLogic scenario-parameter configuration table
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{figures/chapter4/fig_4_3_passenger_flow_architecture.png}
    \caption[AnyLogic passenger arrival flow architecture]{Passenger arrival flow architecture implemented in the AnyLogic simulation model. The discrete-event model represents the passenger arrival journey at AIBD through five operational stages: arrival generation, immigration control, baggage reclaim, customs inspection, and passenger exit. Within these stages, queue accumulation, congestion propagation, and throughput performance are dynamically evaluated. Source: AnyLogic simulation model developed for this study.}
    \label{fig:chapter4-passenger-flow-architecture}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        height=0.78\textheight,
        keepaspectratio
    ]{figures/chapter4/fig_4_4_dynamic_intervention_framework.png}
    \caption[Dynamic intervention framework for congestion management]{Dynamic intervention framework for adaptive airport congestion management. Real-time queue monitoring is followed by threshold evaluation; when congestion is detected, additional operational capacity is activated, whereas under normal conditions baseline resources are maintained. Both pathways are intended to improve passenger-flow performance. Source: AnyLogic simulation model developed for this study.}
    \label{fig:chapter4-dynamic-intervention-framework}
\end{figure}
...
\begin{table}[H]
  \centering
  \renewcommand{\arraystretch}{1.25}
  \caption[AnyLogic KPI definitions]{AnyLogic KPI definition table (\texttt{data/\allowbreak processed/\allowbreak anylogic/\allowbreak anylogic\_kpi\_definitions.csv}).}
...
\begin{figure}[htbp]
    ...
    \includegraphics[
        width=\textwidth,
        keepaspectratio
    ]{figures/chapter4/fig_4_5_dss_operational_overview.png}
    \caption[Power BI Operational Overview]{Power BI Decision Support System: Operational Overview page, showing headline KPI cards, the hourly passenger-pressure forecast with the 160-passenger threshold reference line, hourly baggage demand, and the operational-alerts panel (\nolinkurl{dashboards/powerbi/aibd_arrival_operations_dss.pbix}).}
...
\begin{figure}[htbp]
    ...
    \includegraphics[
        width=\textwidth,
        keepaspectratio
    ]{figures/chapter4/fig_4_6_dss_scenario_comparison.png}
    \caption[Power BI Scenario Comparison]{Power BI Decision Support System: Scenario Comparison page. Displays KPI cards for best throughput, lowest queue and resilience score; passenger throughput and queue length by operational scenario; and the Operational Scenario Ranking and Recommendation Matrix, which assigns each scenario a qualitative risk level (Moderate, High or Critical) and a recommendation status (Recommended, Acceptable or Not Recommended). Underlying data source: \nolinkurl{scenario_kpi_results.csv} (Table C.12); \nolinkurl{dashboards/powerbi/aibd_arrival_operations_dss.pbix}.}
...
\begin{figure}[htbp]
    ...
    \includegraphics[
        width=\textwidth,
        keepaspectratio
    ]{figures/chapter4/fig_4_7_dss_dynamic_intervention_monitoring.png}
    \caption[Power BI Dynamic Intervention Monitoring]{Power BI Decision Support System: Dynamic Intervention Monitoring Dashboard. Displays real-time risk status, passenger and baggage peak indicators, capacity-response status, hourly passenger-pressure forecast, real-time baggage processing monitoring, operational alert summary, and the Operational Scenario Ranking and Recommendation Matrix (\nolinkurl{dashboards/powerbi/aibd_arrival_operations_dss.pbix}).}
...
\begin{figure}[htbp]
    ...
    \includegraphics[
        width=\textwidth,
        keepaspectratio
    ]{figures/chapter4/fig_4_9_dss_operational_intelligence_insights.png}
    \caption[Power BI Operational Intelligence Insights]{Power BI Decision Support System: Operational Intelligence Insights page. Combines the Power BI Key Influencers visual for congestion risk with an interpretation panel of key findings and operational recommendations. The influence results shown here are the basis of the ranked factors later reported in Table C.15 (\nolinkurl{dashboards/powerbi/aibd_arrival_operations_dss.pbix}).}
